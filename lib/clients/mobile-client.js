...
const { paddedLength, xorBytes, stripPadding, generateNonce } = require('../utils');
const { StateError, GatewayError } = require('../errors');
...
class KeyBag {
    ...
    constructor(encrypted) {
        const decrypted = createDecipher(getKey('MOBILE_GW_KEY')).update(encrypted, 'hex').toString();
...
class MobileClient extends GatewayClient {
    // Readonly properties: only set in the constructor
    #userAgent;
    #lang;
    #deviceInfo;
    // Session properties: set before the user logs in.
    #mediaUrl;
    #keys;
    #sid;
    // User properties: must be reset when the user is logged out or changes,
    #userInfo;
    #license;
    #mediaClient;
    #authToken;
     ...
     * @param {Object} userInfo This parameter is deprecated and will be removed in a future release.
    ...
    constructor(userInfo = {}, setup = DEFAULTS) {
        if (Object.keys(userInfo).length) {
            console.warn('warn: MobileClient constructor with userInfo is deprecated, and will be removed in a future release.');
        }
        super(setup.baseUrl);
        this.#keys = null;
        this.#sid = null;
    ...
    _setInstanceSearchParams(url, gatewayMethod, httpMethod) {
        url.searchParams.set('api_key', getKey('MOBILE_API_KEY'));
    ...
    async initializeKeys() {
        // Get the token
        const uniqId = generateNonce(32);
        const mobileAuthResponse = await this._callGatewayMethod('mobile_auth', {}, 'get', {
            uniq_id: uniqId,
        });
        this.#mediaUrl = mobileAuthResponse.CONFIG.URL_MEDIA;
    ...
    /**
     * Gets the MediaClient for the current user.
     */
    get mediaClient() {
        if (!this.#mediaClient) {
            if (!this.#license || !this.#mediaUrl) {
                return undefined;
            } 
            this.#mediaClient = new MediaClient(this.#license.LICENCE.OPTIONS.license_token, this.#mediaUrl);
        }
        return this.#mediaClient;
    }

    /**
     * Retrieves the AuthToken for the current session, generating it if necessary.
     * @returns The current AuthToken.
     */
    async retrieveAuthToken() {
        if (this.#authToken) return this.#authToken;

        if (this.userInfo.arl) {
            this.#authToken = await AuthToken.getTokenFromArl(this.userInfo.arl);
        } else {
            this.#authToken = await AuthToken.getAnonymousToken();
        }
     ...
     * @returns {Promise<Boolean>} True if sign in was successful, false if incorrect credentials were given.
     */
    ...
    /**
     * Signs in with a userInfo object from a previous session.
     * @param {object} userInfo An object with the `arl` and `id` of the previous session.
     */
    async signInWithUserInfo(userInfo) {
        this.#prepareForLogin();
    ...
    /**
     * Invalidates the auth token and resets the client state.
     */
    async signOut() {
        if (this.#authToken) {
            await this.#authToken.logout();
            this.#authToken = undefined;
        }
        this.#prepareForLogin();
    }
     ...
     * @deprecated Use signInWithArl instead.
     */
    async restoreSession() {
        console.warn('warn: restoreSession is deprecated and will be removed in a future version. Use signInWithArl instead.');
        if (!this.signedIn) {
        ...
        try {
            await this.signInWithUserInfo(this.#userInfo);
        } catch(e) {
